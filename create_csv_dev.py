...
for part in parts:
    ...
    test_label_path = os.path.join(part_path, "dev_label.txt")
    
    file = open(test_label_path, 'rt')
    Lines = file.readlines()
    for line in Lines:
        file_name, label = line.strip().split(" ")
        path = os.path.join("datasets/phase1", file_name)
    ...
    train_df.to_csv('code/data/dev_{}.csv'.format(part), index=False)