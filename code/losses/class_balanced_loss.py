...
def CB_loss(labels, logits, samples_per_cls, no_of_classes, loss_type, beta, gamma, device, label_type="hard_label"):
    ...
    if label_type == "soft_label":
      _, hard_labels = torch.max(labels, 1)
      labels_one_hot = F.one_hot(hard_labels, no_of_classes).to(device).float()
    else:
      labels_one_hot = F.one_hot(labels, no_of_classes).to(device).float()
